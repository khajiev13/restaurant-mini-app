...
from app.schemas.table import TableContextResponse, TableManifestItem, TableResolveRequest
...
async def resolve_table(body: TableResolveRequest) -> ApiResponse:
    try:
        ...
        message = str(exc)
        code = status.HTTP_404_NOT_FOUND if message == "Table code was not found" else status.HTTP_400_BAD_REQUEST
...
async def get_table_manifest(current_user: CurrentUserDep) -> ApiResponse:
    ...
    items = [TableManifestItem.model_validate(item).model_dump() for item in await table_access.manifest()]
